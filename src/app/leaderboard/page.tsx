...
type Ranking = { rank:number; username:string; name_style:string; net_worth:number; badges:PublicBadge[]; public_lineup:boolean; is_current_user:boolean };
...
export default function LeaderboardPage() {
  ...
  return <div className="app-shell"><Header/><main className="container page-content global-leaderboard-page">
    ...
    {results&&<section className="leaderboard-search-results" aria-live="polite"><h2>{text.search}</h2>{results.length?<div className="leaderboard-list">{results.map(row=><article key={row.username}><span className="rank">#{row.rank}</span><span className="leaderboard-manager"><strong className={`name-${row.name_style}`}>{row.username}</strong><BadgeList badges={row.badges} compact/>{row.public_lineup?<button className="lineup-cta" onClick={()=>showLineup(row.username)}>{lineupText.view}</button>:<small className="lineup-private">{lineupText.private}</small>}</span><b>{format(row.net_worth)}</b></article>)}</div>:<p className="empty-state compact">{text.noMatch}</p>}</section>}
