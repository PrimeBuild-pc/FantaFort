...
export default function WalletPage() {
  ...
  const label=(type:string)=>t(({
    initial_grant:'initialGrant',migration:'migratedBalance',trade_buy:'tradeBuy',trade_sell:'tradeSell',daily_rescue:'dailyRescue',gift_sent:'giftSent',gift_received:'giftReceived',league_lock:'leagueLock',league_refund:'leagueRefund',league_prize:'leaguePrize',league_loss:'leagueLoss'
  ...
  return <div className="app-shell"><Header/><main className="container page-content"><div className="page-title"><div className="eyebrow">ACCOUNT ECONOMY</div><h1>{t('wallet')}</h1><p>{t('virtualCoinsNotice')}</p><p className="form-hint"><Link href="/account">{t('topUp')} · {t('cosmetics')} →</Link></p></div>
