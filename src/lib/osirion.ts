...
function isCompetitive(eventId:string){const id=eventId.toLowerCase();if(/mobile|console|ranked|playstation|test|performance|zerobuild|trioszb|soloszb/.test(id))return false;if(id.includes('divisionalcup')&&!id.includes('division1'))return false;return /fncs|cashcup|victorycup|elite|divisionalcup/.test(id)}
