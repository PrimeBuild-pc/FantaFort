...
Each encrypted archive and the upload artifact are named with environment, run ID and run attempt, so a manually re-run failed job never collides with the prior attempt's artifact. Before and after every dump attempt, the workflow calls the Supabase Management API (`POST /v1/projects/{ref}/database/query`, authenticated with the same access token — no DB password or connection string involved) to terminate any residual `pg_dump` backend still open once the CLI subprocess has returned; a legitimate dump session cannot outlive that call, so anything left is orphaned. If termination doesn't actually clear the session, the job fails immediately instead of retrying or proceeding. A failed link/dump attempt is retried at most once (`BACKUP_MAX_ATTEMPTS`) and only after that verified cleanup. `supabase` CLI output is captured to a temp log and only surfaced on failure, redacted for `postgres://` connection strings and `password=` values first, since a dynamically issued DB password from the CLI is not a registered GitHub secret and would otherwise print unmasked.

**Session-identity limit:** `supabase db dump --linked` (CLI source checked, `pkg/migration/dump.go`) has no flag or env var to set `application_name`, so the cleanup cannot tag a session with a verified per-run/attempt identity without switching to `--db-url` with a hand-built connection string — which would require fetching/handling the raw DB password ourselves, the exact risk this workflow avoids. Instead, cleanup is scoped to `application_name = 'pg_dump' AND usename LIKE 'cli\_login\_%'`: as long as `DB_PASSWORD`/`-p` is never set (it isn't here), `--linked` always authenticates through a Management-API login role minted fresh per invocation (`internal/utils/flags/db_url.go`'s `initLoginRole`), never the static `postgres` role or an application runtime role — so this can only match a Supabase-CLI-driven session, never app traffic, a manual dump run under `postgres`, or a Supavisor/PgBouncer system connection. It does not rule out a concurrent, out-of-band `supabase db dump --linked` run by a human against the same project in the same window, since that would also authenticate as a `cli_login_*` role indistinguishable from ours; this workflow is the only sanctioned user of `supabase db dump --linked` against these projects.
